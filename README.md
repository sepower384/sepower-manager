...
7. **발행** — 기본은 **자동 발행**이다.
   - 07~24시에만, 22~40분 사이 무작위 간격으로 올린다(하루 최대 30개).
   - 올라간 글은 강회장 DM으로 사본이 오고, 🗑 버튼으로 채널에서 바로 지울 수 있다.
   - 발행 채널(`TELEGRAM_TARGET_CHAT_ID`)이 비어 있으면 초안을 DM으로만 보낸다.
   - `/approve`로 바꾸면 초안마다 ✅ 올리기 / 🔁 다시쓰기 / 🗑 버리기 버튼이 온다.
     - 초안에 답장으로 글을 보내면 그 글로 교체된다. `!더 짧게`처럼 보내면 그 지시대로 다시 쓴다.
   - 6시간 넘게 못 나간 글은 시의성이 떨어져 만료된다.
