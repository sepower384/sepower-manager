...
MIN_BYTES = 25000          # 이보다 작으면 빈 화면·로고일 가능성
...
FIND_HEADLINE_JS = """
() => {
  const hs = [...document.querySelectorAll('h1, h2')].filter(h => {
    const r = h.getBoundingClientRect();
    return r.width > 200 && r.height > 20 && h.innerText.trim().length > 8;
  });
  if (!hs.length) return null;
  hs.sort((a, b) => parseFloat(getComputedStyle(b).fontSize) - parseFloat(getComputedStyle(a).fontSize));
  const h = hs[0];
  const r = h.getBoundingClientRect();
  ...
  return {x: Math.max(0, r.left - 40), y: r.top + window.scrollY, w: Math.max(640, w + 80)};
}
"""
...
def screenshot_article(url, name):
    ...
    with sync_playwright() as p:
        ...
        head = pg.evaluate(FIND_HEADLINE_JS) or {"x": 0, "y": 40, "w": 1200}
        x = head["x"]
        w = min(1200 - x, head["w"])
        pg.evaluate("y => window.scrollTo(0, y)", max(0, head["y"] - 40))
        pg.wait_for_timeout(400)
        top = max(0, head["y"] - 40 - pg.evaluate("() => window.scrollY"))
        h = min(round(w * 9 / 16), 900 - top)
        pg.screenshot(path=path, clip={"x": x, "y": top, "width": w, "height": h})
        final = pg.url
        b.close()
...
def image_for(item, draft_id, used_hashes=()):
    ...
    if item["source"] == "news":
        ...
        tries.append(lambda: screenshot_article(url, base + "_cap.png"))
        if item["photos"]:
